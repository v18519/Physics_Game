...
public class BasicPolygon  {
	...
	public BasicPolygon(float sx, float sy, float vx, float vy, float width, float height, Color col, float mass, float rollingFriction, int numSides) {
		this(sx, sy, vx, vy, width, height, col, mass, rollingFriction,mkRegularPolygon(width, height),numSides);
	}
	public BasicPolygon(float sx, float sy, float vx, float vy, float width, float height, Color col, float mass, float rollingFriction, Path2D.Float polygonPath, int numSides) {
		...
		//pos =bodyDef.position.set(sx, sy);
		velo =bodyDef.linearVelocity.set(vx, vy);
	...
	public Vec2 pos() {
		...
		Vec2 reset = bodyDef.position.set(0.3f, 3.3f);
	...
	public Vec2 update() {
			...
			//System.out.println("poly x : " + pos1.x+ "poly y : " + pos1.y);
			return pos1;
	...
	public void draw(Graphics2D g) {
		...
		//af.translate(-camera.getPosition().x + 400, -camera.getPosition().y + 300);
		...
		g.fill(p);
		//g.translate(-Camera.getPosition().x + 400, -Camera.getPosition().y + 300);
	...
	public static Path2D.Float mkRegularPolygon(float width, float height) {
		...
		p.moveTo(width*(-0.755f),height*( -0.25f));
		p.lineTo(width*(0.75f), height*( -0.25f));
		p.lineTo(width*0.75f, height*0.0f);
		p.lineTo(width*0.0f, height*0.45f);
		p.lineTo(width*(-0.575f), height*0.45f);
		p.lineTo(width*(-0.735f), height*0.1f);
