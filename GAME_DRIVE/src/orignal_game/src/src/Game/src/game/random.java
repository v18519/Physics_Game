...
public class random  {							//////// test particle
	...
	public random(float sx, float sy, float vx, float vy, float radius, Color col, float mass, float rollingFriction, BodyType x, float linearDamping, float friction) {
		...
		// float rand = MathUtils.randomFloat(-2.0f, 2f);
		float rand = (float) Math.random();
		    bodyDef.position.set(sx+rand, sy+rand);
